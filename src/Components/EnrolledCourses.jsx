            ...
            <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                <div class="mt-5 sm:mt-0">
                    ...
                    <div className='flex mt-1'>
                        <Rating rating={item.rating} top={1}/>
                        <p className='text-xs relative bottom-0.5 ms-2'>({item.students})</p>
                    </div>
                    ...
                    <div className='relative lg:hidden md:hidden sm:hidden  lg:top-6 md:top-6 bottom-10 left-36 xs:block'>
                        <div className="radial-progress text-success" style={{ "--value": item.progress, fontSize: "12px", "--size": "2.5rem", "--thickness": "4px" }}>{item.progress ? item.progress : null}%</div>
                    </div>
                </div>
                <div className='relative lg:block md:block sm:block  lg:top-6 md:top-6 sm:top-6 lg:left-2 xs:hidden'>
                    <div className="radial-progress text-success" style={{ "--value": item.progress, fontSize: "12px", "--size": "2.5rem", "--thickness": "4px" }}>{item.progress ? item.progress : 0}%</div>
                </div>
            </div>
        </div>
    )
